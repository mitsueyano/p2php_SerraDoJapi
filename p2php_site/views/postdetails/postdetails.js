...
function loadPostData() {
    ...
    if (!postId) {
        document.getElementById('post-container').innerHTML = '<p class="error">ID do registro não fornecido</p>';
        return;
    }
        ...
        <div class="post-detail">
            <div class="post-header">
                <div class="author-info">
                    <img src="${postData.usuario.imagem_perfil || '../img/default-profile.png'}" alt="Foto de perfil" class="profile-pic">
                    <div>
                        <h2>${postData.usuario.nome} ${postData.usuario.sobrenome}</h2>
                        <span class="username">@${postData.usuario.username}</span>
                        ${postData.usuario.nivel_acesso === 'especialista' ? '<span class="badge">Especialista</span>' : ''}
                    </div>
                </div>
                <div class="post-meta">
                    <span class="post-date">${formatDateTime(postData.data_publicacao, postData.hora_publicacao).replace(',', ' às')}</span>
                </div>
            </div>
            ...
            <div class="post-content">
                <div class="post-image">
                    ${postData.imagem_url ?
                        `<img src="${postData.imagem_url}" alt="${postData.taxonomia.nome_popular}" onclick="openImageModal('${postData.imagem_url}')">` :
                        '<div class="no-image"><i class="fas fa-camera"></i><span>Sem imagem</span></div>'}
                </div>

                <div class="post-info">
                    <h1 class="species-name">${postData.taxonomia.nome_popular}</h1>
                    <p class="scientific-name">${postData.taxonomia.especie}</p>
                    
                    <div class="taxonomy-info">
                        <div><span class="label">Classe:</span> ${postData.taxonomia.classe}</div>
                        <div><span class="label">Ordem:</span> ${postData.taxonomia.ordem}</div>
                        <div><span class="label">Família:</span> ${postData.taxonomia.familia}</div>
                        <div><span class="label">Categoria:</span> ${postData.taxonomia.categoria}</div>
                    </div>
                    ...
                    <div class="observation-info">
                        <p><strong>Data da observação:</strong> ${formatDateTime(postData.data_observacao, postData.hora_observacao)}</p>
                        ${postData.identificacao ? '<span class="tag identified">Verificado por especialista <i class="fa-solid fa-check"></i></span>' : ''}
                        ${postData.especie_invasora ? '<span class="tag invasive">Espécie invasora</span>' : ''}
                    </div>
                ...
                <div class="post-interactions">
                    <button class="like-btn ${postData.liked ? 'liked' : ''}" onclick="toggleLike(${postData.id})">
                        <i class="${postData.liked ? 'fas' : 'far'} fa-heart"></i>
            ...
            <div class="comments-section">
                ...
                <div class="comment-form">
                    ...
                    <button onclick="postComment(${postData.id})">Enviar</button>
                </div>
    ...
    return comments.map(comment => `
        <div class="comment ${comment.respostas.length ? 'has-replies' : ''}">
            <div class="comment-header">
                <img src="${comment.usuario.imagem_perfil || '../img/default-profile.png'}" alt="Foto de perfil" class="comment-profile-pic">
                <div>
                    <span class="comment-author">${comment.usuario.nome} ${comment.usuario.sobrenome}</span>
                    <span class="comment-username">@${comment.usuario.username}</span>
                    ${comment.usuario.nivel_acesso == "especialista" ? '<span class="badge">Especialista</span>' : ''}
                </div>
                <span class="comment-date">${new Date(comment.data_publicacao).toLocaleString('pt-BR', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
            ...
            <div class="reply-form-container" id="reply-form-${comment.id}" style="display: none;">
                ...
                <button onclick="postReply(${comment.id}, ${comment.id_registro})">Enviar resposta</button>
            </div>
                ...
                <div class="replies">
                    ${comment.respostas.map(reply => `
                        <div class="comment reply">
                            <div class="comment-header">
                                <img src="${reply.usuario.imagem_perfil || '../img/default-profile.png'}" alt="Foto de perfil" class="comment-profile-pic">
                                <div>
                                    <span class="comment-author">${reply.usuario.nome} ${reply.usuario.sobrenome}</span>
                                    <span class="comment-username">@${reply.usuario.username}</span>
                                    ${reply.usuario.nivel_acesso == "especialista" ? '<span class="badge">Especialista</span>' : ''}
                                </div>
                                <span class="comment-date">${new Date(reply.data_publicacao).toLocaleString('pt-BR', {
                                    day: '2-digit',
                                    month: '2-digit',
                                    year: 'numeric',
                                    hour: '2-digit',
...
function openImageModal(imageUrl) {
    // Implementar modal para visualização da imagem
    ...
    .then(data => {
        if (data && data.success) {
            ...
            newComment.innerHTML = `
                <div class="comment-header">
                    <img src="${data.userData.pfp}" alt="Foto de perfil" class="comment-profile-pic">
                    <div>
                        <span class="comment-author">${data.userData.name}</span>
                        <span class="comment-username">@${data.userData.username}</span>
                        ${data.userData.isSpecialist ? '<span class="badge">Especialista</span>' : ''}
                    </div>
...
function postReply(commentId, postId) {
    ...
    .then(data => {
        ...
        const parentComment = document.querySelector(`#reply-form-${commentId}`).closest('.comment');
        ...
        replyElement.innerHTML = `
            <div class="comment-header">
                <img src="${data.userData.pfp || '../img/default-profile.png'}" alt="Foto de perfil" class="comment-profile-pic">
                <div>
                    <span class="comment-author">${data.userData.name}</span>
                    <span class="comment-username">@${data.userData.username}</span>
                    ${data.userData.isSpecialist ? '<span class="badge">Especialista</span>' : ''}
